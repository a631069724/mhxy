...
class Shimen(Base):
    ...
    def Run(self):
        ...
        self.Begin()
        #领取师门任务
        if self.find(self.EventLingqu,0.5):
    ...
    def isDone(self):
        self.flush()
        #同一任务 防止点击频繁被系统判断脚本
        if  self.find(self.EventShimenRenwu,0.9):
        ...
        if self.find(self.EventShiMenDone):
            ...
            return True
        if self.isHomePage() \
                and self.find(self.EventShimenRenwu,0.9) is None \
                and self.find(self.EventXuanzeyaozuodeshi) is None: #防止对话弹窗遮挡
            return True
        return False
